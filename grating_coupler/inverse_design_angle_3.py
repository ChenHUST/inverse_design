...
start_angle = 0
start_index = int((start_angle + ff_angle) / d_angle)
stop_angle = 25
stop_index = int((stop_angle + ff_angle) / d_angle)
...
def J3(*FF):
    ...
    error = npa.abs(npa.abs(points) ** 2 - final_intensity)
    return npa.sum(error[start_index : (stop_index + 1)])
...
np.save("evaluation_history_6.npy", evaluation_history)
