...
scatter_angles = [-22.5, -20, -17.5, -15, -12.5, -10, -7.5, -5, -2.5, 0]
P_angles_scatter = np.load(config_path + "/P_angles_scatter.npy")
P_angles_in = np.load(config_path + "/P_angles_in.npy")
P_angles_out = np.load(config_path + "/P_angles_out.npy")
# plt.plot(P_angles_scatter, "-o")
# plt.show()
######################## 导入P_angles_scatter ############################

# for ii in range(0, len(scatter_angles)):
for ii in [9]:
    # !需要修改
    update_factor = 100  # 迭代次数
    scatter_angle = scatter_angles[ii]  # 散射角度
    input_scale = 1 / np.sqrt(P_angles_in[ii])  # 输入比例
    idea_output = P_angles_out[ii]  # 输出
    ...
    sources[0].amplitude = (
        1 / input_scale / mp.GaussianSource(fcen, fwidth=fwidth).fourier_transform(fcen)
    )
    ...
    init_para = (
        np.abs(
            np.load(
                config_path
                + "/init_segmented_opt/"
                + f"final_eps_seg_init_{scatter_angle}.npy"
            )[0:number_para, 5]
        )
        - 1.44**2
    ) / (1.96**2 - 1.44**2)
    ...
    np.save(f"eval_history_seg_angle_{scatter_angle}.npy", evaluation_history)
    ...
    np.save(f"final_eps_seg_angle_{scatter_angle}.npy", eps)
